---
title: 'Demultiplexing the BAL pool: BP2'
author:
- name: Anson Wong
date: "`r Sys.Date()`"
description: null
...
```{r}
table(obj.list$`BP2-c1`$HTO_classification.global)
...
```{r}
table(obj.list$`BP2-c2`$HTO_classification.global)
...
```{r}
table(obj.list$`BP2-c3`$HTO_classification.global)
...
Idents(obj.list$`BP2-c1`) <- "hash.ID"
RidgePlot(obj.list$`BP2-c1`, assay = "HTO", 
          features = rownames(obj.list$`BP2-c1`[["HTO"]]))
...
```{r fig.width=24,fig.height=12}
# Group cells based on the max HTO signal
Idents(obj.list$`BP2-c2`) <- "hash.ID"
RidgePlot(obj.list$`BP2-c2`, assay = "HTO", 
          features = rownames(obj.list$`BP2-c2`[["HTO"]]))
...
```{r fig.width=24,fig.height=12}
# Group cells based on the max HTO signal
Idents(obj.list$`BP2-c3`) <- "hash.ID"
RidgePlot(obj.list$`BP2-c3`, assay = "HTO", 
          features = rownames(obj.list$`BP2-c3`[["HTO"]]))
...
HTOHeatmap(obj.list$`BP2-c1`, assay = "HTO", ncells = 5000) + 
  ggtitle("BP2-c1") + 
...
```{r fig.width=8,fig.height=6}
HTOHeatmap(obj.list$`BP2-c2`, assay = "HTO", ncells = 5000) + 
  ggtitle("BP2-c2") + 
...
```{r fig.width=8,fig.height=6}
HTOHeatmap(obj.list$`BP2-c3`, assay = "HTO", ncells = 5000) + 
  ggtitle("BP2-c3") + 
  theme(plot.title = element_text(hjust=0.5, size=12))
...
hashtag.subset <- subset(obj.list$`BP2-c1`, idents = "Negative", invert = TRUE)
...
```{r}
# remove negative cells from the object
hashtag.subset <- subset(obj.list$`BP2-c2`, idents = "Negative", invert = TRUE)
...
```{r}
# remove negative cells from the object
hashtag.subset <- subset(obj.list$`BP2-c3`, idents = "Negative", invert = TRUE)
